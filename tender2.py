...
def fetch_sheet_data():
    try:
        ...
        # Adjusted: Headers in row 3 (0-indexed: values[2]), data from row 4 onwards (values[3:])
        headers = values[2] if len(values) > 2 else [''] * MAX_COLS
        raw_data_rows = values[3:] if len(values) > 3 else []
