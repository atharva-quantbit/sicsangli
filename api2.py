...
# SERVICE_ACCOUNT_FILE = "/home/erpadmin/asp-demo-v16/sites/assets/sicsangli/js/credentials.json"
# SHEET_NAME = "sheet3"
...
#                 header_type = 'both'
#             headers.append({'name': name, 'type': header_type})
#     return headers
...
#     for col_index, header in enumerate(header_types, 2):
#         if header['type'] in ['kpi', 'both']:
#             col_values = []
#             for row in rows:
...
#                 kpis.append({'label': header['name'], 'value': round(total_sum, 2)})
#     return kpis
...
#                 chart_type = chart_types[(col_index - 2) % len(chart_types)]
#                 charts.append({
#                     'type': chart_type,
#                     'label': f"{header['name']} Chart",
#                     'labels': labels,
#                     'data': data
#                 })
#     return charts
...
#             # Skip yellow totals row
#             if i < len(sheet_data) and is_yellow_row(sheet_data[i]):
#                 i += 1

#             # Calculate KPIs and charts
#             kpis = calculate_kpis(headers_with_type, data_rows)
#             charts = prepare_charts(headers_with_type, data_rows)

#             table = {
#                 'title': title,
#                 'headers': headers,
#                 'header_types': headers_with_type,
#                 'rows': data_rows,
#                 'kpis': kpis,
#                 'charts': charts
#             }
